            ...
            <div class="thing-infos">
              ...
              <div class="thing-info-box">
                <div class="thing-state">
                  <span class="state hidden-sm">Status</span>
                  <span>valid</span>
                </div>
                <h1 class="thing-name">{{ detailData.title }}</h1>
                <span>
                  <span class="a-price-symbol"></span>
                  <span class="a-price" style="font-size: 20px;">{{detailData.price}} ￥/H</span>
                </span>
                <div class="translators flex-view" style="">
                  <span>Subject: </span>
                  <span class="name">{{ detailData.classificationTitle }}</span>
                </div>
                <div class="translators flex-view" style="">
                  <span>Gender:  </span>
                ...
                <div class="translators flex-view" style="">
                  <span>Age:  </span>
                  <span class="name">{{ detailData.age }}years old</span>
                </div>
                <div class="translators flex-view" style="">
                  <span>District: </span>
              ...
              <div class="count-item flex-view pointer" @click="addToWish()">
                ...
                <div class="count-box flex-view">
                  <div class="count-text-box">
                    <span class="count-title">Hiring</span>
                  </div>
          ...
          <span class="tab"
            ...
            <div class="thing-comment" :class="selectTabIndex > 0? '':'hide'">
              ...
              <div class="publish flex-view">
                ...
                <input placeholder="Say something..." class="content-input" ref="commentRef">
                <button class="send-btn" @click="sendComment()">Send</button>
              </div>
              <div class="tab-view flex-view">
                <div class="count-text">Total {{ commentData.length }} comments</div>
                <div class="tab-box flex-view" v-if="commentData.length > 0">
                  <span :class="sortIndex === 0? 'tab-select': ''" @click="sortCommentList('recent')">Latest</span>
                  <div class="line"></div>
                  <span :class="sortIndex === 1? 'tab-select': ''" @click="sortCommentList('hot')">Popular</span>
                </div>
              </div>
                ...
                <div class="comment-item" v-for="item in commentData">
                  <div class="flex-item flex-view">
                    ...
                    <div class="float-right">
                      <span @click="like(item.id)">agree</span>
...
let tabData = ref(['Introduction', 'Reviews'])
...
const addToWish =()=> {
  ...
  if (userId) {
    wishApi({thingId: thingId.value, userId: userId}).then(res => {
      ...
      getThingDetail()
      createDialogApi(userId, detailData.value.userId)
    }).catch(err => {
      console.log('Failed')
    })
  } else {
...
const collect =()=> {
  ...
  if (userId) {
    ...
    message.warn('Please firstly login！')
  }
}
...
const handleOrder =(detailData)=> {
  ...
  if(userId){
    Modal.info({
      title: 'Contact Way',
      content: 'Phone number：'+detailData.mobile,
      onOk() {
      },
    });
  }else{
